...
###### NOTE: This is a hobby project  to explore game engine development. As such, this repository may be updated at random intervals. The code may not be the most efficient or safe. 

## Game structures and related functions
### Scene
...
#### drawScene
```drawScene(GXscene_t* scene)``` will draw all objects in the linked list, so long as the ```GXE_drawable``` flag is set in the ```flags``` member of the respective entity.
#### getEntity
...
#### drawEntity
```int drawEntity (GXentity_t* entity);``` will draw ```entity```, as long as the ```GXE_drawable``` flag is set in ```flags```.
